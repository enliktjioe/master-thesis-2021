...
def prepareResult(unlabelSet, predict, vocabulary):
	...
	for i in range(len(predict)):
		if(predict[i] == 1):
			...
			unlabelSet[i].label = 0
			uninformRev.append(unlabelSet[i])


	informMat = toCOOMatrix(informRev, vocabulary)

	return informRev, uninformRev, informMat
...
def AR_emnb(trainSet, testSet, unlabelSet, vocabulary, dataSetName):
	...
	informRev, uninformRev, informMat= prepareResult(unlabelSet, predict, vocabulary)
