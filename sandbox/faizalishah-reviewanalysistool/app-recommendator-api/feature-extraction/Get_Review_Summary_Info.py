...
class ReviewSummarizer:
	...
	def ExtractNounTermsFromAppDescription(self):
		r = requests.get('http://localhost:8081/appInfo?id=' + self.appID)
		output = r.json()
		appDesciption= output['description']
		appTitle = output['title']
		appIconUrl = output['iconUrl']
	...
	def ReviewBatchSummaryInfo(self,txt_AllReviews):
		...
		res = core_nlp.annotate(txt_AllReviews, properties={'annotators': 'tokenize,ssplit,pos,parse,sentiment','outputFormat': 'json','timeout': '500000000',"parse.model": "edu/stanford/nlp/models/srparser/englishSR.ser.gz"})
		...
		if len(lst_clean_sents)>0:
			...
			for i in range(0,len(lst_review_sents)):
				...
				if review_sent_class=='E':
					...
					dict_review_sentence_info = {'sent_id': sent_id, 'sent_words': sent_words, 'sent_stemmed_words':stemmed_words,'category': review_sent_class, 'feature_terms': [], 'sentiment': str(scaled_sent_score), "appID" : self.appID}
					lst_relevant_review_sents.append(dict_review_sentence_info)
				elif review_sent_class=='B':				
					sentiment_score = int(-2)
					dict_review_sentence_info = {'sent_id': sent_id, 'sent_words': sent_words, 'sent_stemmed_words':stemmed_words,'category': review_sent_class, 'feature_terms': [], 'sentiment': str(sentiment_score), "appID" : self.appID}
					lst_relevant_review_sents.append(dict_review_sentence_info)
...
if __name__ == '__main__':
	line = read_in()
	AppIDs = line["AppIDs"]
	NoOfPages = line["pages"]
	AppsReviewSummaries={}
	#print(line)

	for appid in AppIDs:
		objReviewSummarizer = ReviewSummarizer(appid,NoOfPages)
		ReviewsSummaryInfo = objReviewSummarizer.GetAppReviews_SummaryInfo()
		AppsReviewSummaries[appid] = ReviewsSummaryInfo


	# with open('ReviewSummary.json', 'w') as outfile:
	# 	json.dump(AppsReviewSummaries, outfile)
	print(json.dumps(AppsReviewSummaries))
