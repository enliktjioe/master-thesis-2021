...
public class AppPrepareDataFolds {
	...
	public static void main(String[] args) throws JAXBException, IOException, ParseException {
		...
		options.addOption("c", "file", true, "con file");
		...
		String con_dataFile = line.getOptionValue('c');
		...
		List<String> Con_dataLines = FileUtils.readLines(new File(con_dataFile));
		...
		// Create trainSet and testSet from Contineous features 
		for (int i = 0; i < folds; i++){
			...
			counter = 0;
			List<String> record = new ArrayList<String>();
			for (String strLine : Con_dataLines) {
				record.add(strLine);
				if (strLine.isEmpty()) {
					counter++;
			...
			File trainFile = new File(targetDir + "/train" + i + ".con");
			File testFile = new File(targetDir + "/test" + i + ".con");
			
			FileUtils.writeLines(trainFile, trainSet, false);
			FileUtils.writeLines(testFile, testSet, false);
			
			System.out.println("[INFO] Train set (contineous) is written to: " + trainFile.getPath());
			System.out.println("[INFO] Test set (contineous) is written to: " + testFile.getPath());
		}
