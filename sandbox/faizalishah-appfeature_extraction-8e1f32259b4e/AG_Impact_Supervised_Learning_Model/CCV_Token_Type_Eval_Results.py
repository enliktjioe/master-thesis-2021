...
class CRFBIOTagEvaluation:
    def __init__(self,testSet_file,trainSet_file):
        ...
        #self.PerformEvaluation(list_lines)
    ...
    def In_Out_Train_AspectTerms_Evaluation(self):
        ...
        print("In Train [Token-based Evaluation]")
        print(InTrain_evaluation_token_exact)
        print("##########################")
        
        print("Out Train [Token-based Evaluation]")
        print(OutTrain_evaluation_token_exact)
        print("##########################")
        
        print("In Train [Type-based Evaluation]")
        print(InTrain_evaluation_type_exact)
        print("##########################")
        
        print("Out Train [Type-based Evaluation]")
        print(OutTrain_evaluation_type_exact)
        print("##########################")
    ...
    def PerformEvaluation(self):
        ...
        for i in range(0,len(lines)):
            ...
            if len(info) == 2:
                if len(predicted_aspects)!=0 or len(true_aspects)!=0:
                    ...
                    #print(sent_predicted_aspects,sent_true_aspects)
                    #print(tp)
                    #print("++++++")
        ...
        #print(true_predictions,false_predictions,false_negatives)
        ...
        #print(tp_type_exact,fp_type_exact,fn_type_exact)
        ...
        evaluation_exact = {'precision' : '%.3f' % precision_exact ,'recall' : '%.3f' % recall_exact,'fscore' : '%.3f' % fscore_exact}
        evaluation_partial = {'precision' : '%.3f' % precision_partial ,'recall' :  '%.3f' % recall_partial,'fscore' :  '%.3f' % fscore_partial}
        
          #print("TYPE_BASED_EVALAUTION...............")
        evaluation_type_exact = {'precision' : '%.3f' % precision_type_exact ,'recall' : '%.3f' % recall_type_exact,'fscore' : '%.3f' % fscore_type_exact}
        ...
        #exact token
        latex_table = "%.1f & %.1f & %.1f &" % (precision_exact*100,recall_exact*100,fscore_exact*100)
        #partial token
        latex_table += "%.1f & %.1f & %.1f &" % (precision_partial*100,recall_partial*100,fscore_partial*100)
        
        #exact type
        latex_table += "%.1f & %.1f & %.1f &" % (precision_type_exact*100,recall_type_exact*100,fscore_type_exact*100)
        #partial type
        latex_table += "%.1f & %.1f & %.1f" % (precision_type_partial*100,recall_type_partial*100,fscore_type_partial*100)
        ...
        return (evaluation_exact,evaluation_partial,evaluation_type_exact,evaluation_type_partial,all_tp_aspects_exact,latex_table)
...
if __name__ == "__main__":
    ...
    path  = "evaluation_results_latex/" + "result_table.txt"
    f = open(path, 'w')
    ...
    if lang.lower()=="en":
        appCategories=ENGLISH_APPS_CATEGORY
    elif lang.lower()=="de":
        appCategories=GERMAN_APPS_CATEGORY

    
    token_exact_precision,token_exact_recall,token_exact_fscore=0.0,0.0,0.0
    token_partial_precision,token_partial_recall,token_partial_fscore=0.0,0.0,0.0
    
    type_exact_precision,type_exact_recall,type_exact_fscore=0.0,0.0,0.0
    type_partial_precision,type_partial_recall,type_partial_fscore=0.0,0.0,0.0
  
    for appCategory in appCategories:
        print("App Category -> %s" % (appCategory.name))
        filename= appCategory.name + "/" + appCategory.name + "-" + "combine.tsv"
        Train_Filename= appCategory.name + "/" + appCategory.name + "-" + "train.tsv"
        evaluation = CRFBIOTagEvaluation(filename,Train_Filename)
        token_exact,token_partial,type_exact,type_partial,tp_aspects_exact,latex_tbl_row = evaluation.PerformEvaluation()
        tbl_row = appCategory.name.title() + " & " + latex_tbl_row
        
        #token exact
        token_exact_precision += float("{0:.1f}".format(float(token_exact['precision']) * 100))
        token_exact_recall += float("{0:.1f}".format(float(token_exact['recall']) * 100))
        token_exact_fscore += float("{0:.1f}".format(float(token_exact['fscore']) * 100))
        
        #token partial
        token_partial_precision += float("{0:.1f}".format(float(token_partial['precision']) * 100))
        token_partial_recall += float("{0:.1f}".format(float(token_partial['recall']) * 100))
        token_partial_fscore += float("{0:.1f}".format(float(token_partial['fscore']) * 100))
        
        #type exact
        type_exact_precision += float("{0:.1f}".format(float(type_exact['precision']) * 100))
        type_exact_recall += float("{0:.1f}".format(float(type_exact['recall']) * 100))
        type_exact_fscore += float("{0:.1f}".format(float(type_exact['fscore']) * 100))
        
        #type partial
        type_partial_precision += float("{0:.1f}".format(float(type_partial['precision']) * 100))
        type_partial_recall += float("{0:.1f}".format(float(type_partial['recall']) * 100))
        type_partial_fscore += float("{0:.1f}".format(float(type_partial['fscore']) * 100))
            
            
        #exact recall
        
        #print('precision',float(token_exact['precision'])* 100)
        #print(token_exact['precision'],token_exact['recall'])
        #token_exact_precision += float(token_exact['precision'])
        #print(latex_tbl_row)
        #evaluation.GetAspectTermsInTrainSet()
        #evaluation.In_Out_Train_AspectTerms_Evaluation()
        AspectFreqDistribution(tp_aspects_exact,"tp",appCategory.name)
        AspectTermLengthDistribution(tp_aspects_exact,"tp",appCategory.name)
        f.write(tbl_row + '\n')
        #print()
        #print("#####################################################")
        
#     average_exact_token_precision = float("{0:.1f}".format(token_exact_precision/6))
#     average_recall = float("{0:.1f}".format(token_exact_recall/6))
#     average_fscore = float("{0:.1f}".format(token_exact_fscore/6))
    
    f.write('\\bottomrule')
    f.write('\n')
    average_row = '\\textbf{Average} & '
    average_row += "\\textbf{%.1f} & \\textbf{%.1f} & \\textbf{%.1f}" % ((token_exact_precision/6), (token_exact_recall/6),((token_exact_fscore/6)))
    average_row += " & "
    average_row += "\\textbf{%.1f} & \\textbf{%.1f} & \\textbf{%.1f}" % ((token_partial_precision/6), (token_partial_recall/6),((token_partial_fscore/6)))
    average_row += " & "
    average_row += "\\textbf{%.1f} & \\textbf{%.1f} & \\textbf{%.1f}" % ((type_exact_precision/6), (type_exact_recall/6),((type_exact_fscore/6)))
    average_row += " & "
    average_row += "\\textbf{%.1f} & \\textbf{%.1f} & \\textbf{%.1f}" % ((type_partial_precision/6), (type_partial_recall/6),((type_partial_fscore/6)))
    average_row += "\\\\"
    f.write(average_row)
    f.write('\n')
    f.write('\\bottomrule')
