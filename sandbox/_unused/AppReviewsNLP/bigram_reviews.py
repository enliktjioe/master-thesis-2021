...
reviewsRaw['ReviewText'] = reviewsRaw['ReviewText'].astype(str)
reviewsRaw['ReviewLength'] = reviewsRaw['ReviewText'].apply(len)
sns.set_style('white')
g = sns.FacetGrid(reviewsRaw,col='Rating')
g.map(plt.hist,'ReviewLength')
g.savefig("TextLengthStars_paytm.jpeg")

reviewsRaw.drop(['ReviewLength'], axis = 1) 
...
text_for_col2=' '.join(df['POS_filtered_lem2'])
tokens=nltk.word_tokenize(text_for_col2.lower())
bigram_measures = nltk.collocations.BigramAssocMeasures()
finder2 = BigramCollocationFinder.from_words(tokens, window_size=6)
finder2.apply_freq_filter(6)

bigram_likelihood=uniqueTuples(finder2.nbest(bigram_measures.likelihood_ratio, 30))
bigram_raw_freq=uniqueTuples(finder2.nbest(bigram_measures.raw_freq, 30))

for l in bigram_likelihood:
      print(l)


for l in bigram_raw_freq:
      print(l)

length_features2=len(bigram_likelihood)
...
def findfeaturesintext2(text):
     ...
     # In each iteration, add an empty list to the main list
          feature_list.append([])
     i=0
# bigram_likelihood is the list of unique set of bigrams found after collocation
     for l in bigram_likelihood:  
...
df['features2'] = df['POS_filtered_lem'].apply(findfeaturesintext2)

def keepnonempty(list1):
        mylist= [x for x in list1 if x != []]
        return mylist

df['feature']=df['features2'].apply(keepnonempty)
#df.to_csv('senti_file2.csv',index=False)

df_new2=df.feature.apply(pd.Series).merge(df, left_index = True, right_index = True).drop(["feature"], axis = 1).melt(id_vars = ['User','Date','ReviewText','Rating','Text','sentence_sep','POS_filtered','POS_filtered2','POS_filtered_lem','POS_filtered_lem2','features2'], value_name = "features_sep").drop("variable", axis = 1).dropna()

df_new2['features_string'] = [','.join(map(str, l)) for l in df_new2['features_sep']]

df_new2['polarity'] = df_new2['sentence_sep'].apply(sentiment_scores)
#df_new2['polarity'] = df_new2['sentence_sep'].apply(lambda x: TextBlob(x).sentiment[0])
df_new2.to_csv('df_new2.csv',index=False)

x = df_new2.groupby('features_string')['polarity'].mean().sort_values().head(20)
#x.plot(kind='barh')

fig, ax = plt.subplots( nrows=1, ncols=1 )
ax = x.plot(kind='barh', figsize=(15, 12), color='#86bf91', zorder=2, width=0.85)
...
# Set y-axis label
ax.set_ylabel("Bigrams", labelpad=20, weight='bold', size=12)

# Format y-axis label
ax.xaxis.set_major_formatter(StrMethodFormatter('{x:,g}'))

fig.savefig('paytmBigrams.png')   # save the figure to file
plt.close(fig) 
